...
class PubNub {
  public:
     ...
     */
    bool begin(const char *publish_key, const char *subscribe_key,
     ...
     */
    inline PubNub_BASE_CLIENT *publish(const char *channel, const char *message,
        int timeout = 30);
    ...
    /**
     ...
     @param string timeout optional timeout in seconds.
     @return string Stream-ish object with reply message or 0 on error. */
    inline PubNub_BASE_CLIENT *history(const char *channel, int limit = 10,
        int timeout = 310);
  ...
  private:
    enum PubNub_BH {
      PubNub_BH_OK, PubNub_BH_ERROR, PubNub_BH_TIMEOUT,
    };

    inline enum PubNub_BH _request_bh(PubNub_BASE_CLIENT &client,
        unsigned long t_start, int timeout, char qparsep);
...
inline void PubSubClient::_grab_timetoken(uint8_t *nextbuf, size_t nextsize) {
  ...
  enum NTTState {
    await_comma, await_quote, read_timetoken, done
  } state = await_comma;
...
inline PubNub_BASE_CLIENT *PubNub::publish(const char *channel,
    const char *message, int timeout) {
  PubNub_BASE_CLIENT &client = publish_client;
  unsigned long t_start;
  int have_param = 0;

  retry: t_start = millis();
  ...
  if (rslt != 1) {
    DBGprint("Connection error "); DBGprintln(rslt);
    client.stop();
    return 0;
  }
...
inline PubNub_BASE_CLIENT *PubNub::history(const char *channel, int limit,
    int timeout) {
  ...
  while (client.connected() || client.available()) {
    ...
    if (request_state == RS_SKIPLINE) {
      ...
      do {
        ...
        if (linelen
            == strlen_P(
                chunked_str) && !strncasecmp_P(line, chunked_str, linelen)) {
