...
void myBTimerEvent() {
  ...
  if (SunAirPlus_Present) {
    Blynk.virtualWrite(V56,
        String(returnPercentLeftInBattery(BatteryVoltage, 4.19)));
    Blynk.virtualWrite(V57,
        String(returnPercentLeftInBattery(BatteryVoltage, 4.19)) + "%");
    //
    ...
    dtostrf((SolarPanelVoltage * SolarPanelCurrent) / 1000.0, 5, 2,
        floatString);
